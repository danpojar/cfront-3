...
Pnode patch_tree::probe(Pnode n)
{
	...
	ht->action((long)n, 0, Hash::probe, found, replacement);
...
/* node_copy is != node when a fetcher is in use */
    ...
    nodes_seen_hash->action((long)node, 0, Hash::probe, found, old_node);
	...
	if(register_in_hash)
	    nodes_seen_hash->action((long)node,
				    (long)new_node,
				    Hash::insert, zero1, zero2);
    }
    else {
	if(register_in_hash)
	    nodes_seen_hash->action((long)node, (long) node, Hash::insert, zero1, zero2);
    }
    return action;
}
